 ...
 */
cv::Mat un_align_mean(cv::Mat mean, cv::Rect facebox, float scaling_x=1.0f, float scaling_y=1.0f, float translation_x=0.0f, float translation_y=0.0f)
{
    using cv::Mat;
    ...
    aligned_mean_x = ((aligned_mean_x-facebox.x)/facebox.width-translation_x-0.5f)/scaling_x;
    aligned_mean_y = ((aligned_mean_y-facebox.y)/facebox.height-translation_y-0.5f)/scaling_y;
    return aligned_mean;
}
...
// cv::Mat align_mean(cv::Mat mean, cv::Rect facebox, cv::Mat initialisation)
// {
//     using cv::Mat;
//     // Initial estimate x_0: Center the mean face at the [-0.5, 0.5] x [-0.5, 0.5] square (assuming the face-box is that square)
//     // More precise: Take the mean as it is (assume it is in a space [-0.5, 0.5] x [-0.5, 0.5]), and just place it in the face-box as
...
public:
	...
	inline cv::Mat operator()(cv::Mat params) {
        ...
        //std::cout<<rightEyeIdentifiers[0]<<","<<rightEyeIdentifiers[1]<<std::endl;
...
public:
	...
	LandmarkCollection<cv::Vec2f> detect(cv::Mat image, cv::Rect facebox)
	{
        ...
        //cout<<mean_initialisation.type()<<mean_initialisation.at<float>(0,0)<<endl;
        ...
        std::cout<<"right_eye_ids, left_eye_ids"<<right_eye_ids[0]<<","<< left_eye_ids[0]<<std::endl;
	...
	{
//        cv::Mat mean_initialisation = rcr::un_align_mean(initialisation1, facebox);
//        cv::Mat initialisation = rcr::align_mean(mean_initialisation, facebox);
		//rcr::drawLandmarks(image, initialisation, { 0, 0, 255 });
        //cv::Mat mean_initialisation = rcr::align_mean(mean, facebox);
        //std::cout<<"in1ddd:"<<initialisation.at<float>(0,0)<<","<<initialisation.at<float>(0,9)<<std::endl;

        rcr::draw_landmarks(image, initialisation, { 0, 0, 255 });
...
void set_mean(cv::Mat m)///TODO:TFY--set_mean(mat)
{
    mean=m;
};
std::vector<std::string> get_landmark_ids()
{
    return landmark_ids;
};
std::string get_righteye_id1()
{
    return right_eye_ids[1];
};
std::string get_lefteye_id0()
{
    return left_eye_ids[0];
};
private:
...
detection_model load_detection_model(std::string filename)
{
    ...
    ///out-paras
//     std::cout<<"landmark_ids:";
//    for(int i=0;i<rcr_model.landmark_ids.size();i++)
//    {
//        std::cout<<rcr_model.landmark_ids[i]<<",";
//    }
//    std::cout<<std::endl;

//    std::cout<<"right_eye_ids:";
//   for(int i=0;i<rcr_model.right_eye_ids.size();i++)
//   {
//       std::cout<<rcr_model.right_eye_ids[i]<<",";
//   }
//   std::cout<<std::endl;
